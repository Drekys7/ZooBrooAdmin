...
const revision = 'zooweb-main-penguin-photo-v2'
const oldId = 'animal-photo-penguin-v1'
const photoId = 'animal-photo-penguin-v2'

/** Replace the built-in penguin photograph without resetting saved map edits. */
export async function migratePenguinPhoto(database: ZooMapLocalDatabase): Promise<void> {
  ...
  if (!response.ok) throw new Error('Das neue Pinguinfoto konnte nicht geladen werden.')
  ...
  const credit = template.project.items.find(item => item.id === 'map128-penguin')?.imageCredits?.[photoId]
  if (!photo || !credit) throw new Error('Das neue Pinguinfoto ist unvollständig.')
  const data = Uint8Array.from(atob(photo.base64), char => char.charCodeAt(0)).buffer
  if (data.byteLength !== photo.asset.size) throw new Error('Das neue Pinguinfoto ist beschädigt.')
  ...
  await database.transaction('rw', database.projects, database.assets, database.projectMigrations, async () => {
    ...
    const items = project.items.map(item => {
      if (item.id !== 'map128-penguin' || (item.imageAssetId !== oldId && !item.imageAssetIds?.includes(oldId))) return item
      changed = true
